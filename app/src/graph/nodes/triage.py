...
from app.src.llm.prompts import SYSTEM_PROMPT_BANK, TRIAGE_PROMPT, SYSTEM_PROMPT_FINAL_INSTRUCTION
import logging
...
"""
        try:
            response = triage_llm.invoke([SystemMessage(content=system_prompt), *recent_messages], max_tokens=100, temperature=0.3)
        ...
        if response.tool_calls:
            ...
            if not cpf_result['success']:
                state['messages'].extend([response, ToolMessage(content=str(cpf_result), tool_call_id=tool_call["id"])])
                
                prompt = f"""{SYSTEM_PROMPT_BANK}
                    The provided CPF is invalid. Please inform a valid CPF with 11 digits politely.
                    {SYSTEM_PROMPT_FINAL_INSTRUCTION}"""
                
                response_llm = llm.invoke([SystemMessage(content=prompt), *recent_messages], max_tokens=50)
            ...
            final_response = llm.invoke([SystemMessage(content=prompt), *recent_messages], max_tokens=50)
            ...
            if birth_date_result['success']:
                ...
                if not user_is_authenticated['authenticated']:
                    ...
                    if state['authentication_attempts'] >= 3:
                        state['messages'].append(AIMessage(content="Número máximo de tentativas atingido. Encerrando atendimento."))
                        return {**state, "next_agent": END, "birth_date": None, "cpf_input": None}
                    else:
                        state['messages'].append(AIMessage(content="Autenticação falhou. Dados incorretos. Reiniciando processo."))
                        return {**state, "birth_date": None, "cpf_input": None}
                    
                state['birth_date'] = birth_date_result['birth_date']
                state['messages'].extend([response, ToolMessage(content=str(birth_date_result), tool_call_id=tool_call["id"])])
                ...
                try:
                    final_response = llm.invoke([SystemMessage(content=prompt), *recent_messages], max_tokens=100, temperature=0.3)
                except Exception as e:
                    logger.error(f"Error in LLM invocation: {e}")
                    final_response = AIMessage(content="Desculpe, ocorreu um erro ao processar sua solicitação. Tente novamente mais tarde.")
                ...
                state['messages'].append(response)
                try:
                    retry_resp = llm.invoke(f"""{SYSTEM_PROMPT_BANK} Invalid date. Ask again politely. {SYSTEM_PROMPT_FINAL_INSTRUCTION}""", max_tokens=50, temperature=0.2)
                except Exception as e:
                    logger.error(f"Error in LLM invocation: {e}")
                    retry_resp = AIMessage(content="Desculpe, ocorreu um erro ao processar sua solicitação. Tente novamente mais tarde.")
